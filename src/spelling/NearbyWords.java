...
public List<String> suggestions(String word, int numSuggestions) {
    ...
    while(!queue.isEmpty() 
          && (retList.size() < numSuggestions) 
          && (wordsSeen < THRESHOLD)) {
        String nextWord = queue.poll();        
        List<String> tmpRetList = distanceOne(nextWord, true);
        int idx = 0;
        while(idx < tmpRetList.size() && (retList.size() < numSuggestions)) {
            if(!visited.contains(tmpRetList.get(idx))) {                
                visited.add(tmpRetList.get(idx));
                retList.add(tmpRetList.get(idx));
                queue.add(tmpRetList.get(idx));
            }                        
            ++wordsSeen;
            ++idx;            
        }
