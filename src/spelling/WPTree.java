 ...
 * during a Breadth First Search of Nearby words to create a path between
 * two words. 
 * 
...
public List<String> findPath(String source, String target) {
    if(!nw.dict.isWord(source) || !nw.dict.isWord(target)) {
        return new LinkedList<String>();
    }    
    
    this.root = new WPTreeNode(source, null);
    
    HashSet<String> visited = new HashSet<String>();
    LinkedList<WPTreeNode> wordQueue = new LinkedList<WPTreeNode>();
    
    wordQueue.add(root);
    visited.add(source);    
    
    while(!wordQueue.isEmpty()) {        
        WPTreeNode currWord = wordQueue.poll();        
        List<String> mutations = this.nw.distanceOne(currWord.getWord(), true);
        for(String mutation : mutations) {
            if(!visited.contains(mutation)) {
                WPTreeNode childNode = currWord.addChild(mutation);                
                visited.add(mutation);
                if(mutation.equals(target)) {                    
                    return childNode.buildPathToRoot();
                }
                wordQueue.addLast(childNode);
            }
        }
    }    
    return new LinkedList<String>();
